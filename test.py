...
if __name__ == '__main__':
    ...
    plt.title('特征相关性热力图')
    plt.tight_layout()
    plt.savefig('visualizations/features_heatmap.svg', format='svg')
    plt.close()
    print("特征相关性热力图已保存至 visualizations/features_heatmap.svg")
    # 转换为Tensor
    ...
    # 生成模型结构图
    print("\n生成模型结构图...")
    sample_input = torch.randn(1, X_train.shape[1])  # 创建示例输入
    ...
    # 保存为PDF和PNG格式
    dot.format = 'png'
    dot.render('obesity_model_structure', view=False)
    print("模型结构图已保存为 'obesity_model_structure.png'")
    # 训练
